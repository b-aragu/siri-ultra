...
export const handle = async (req: IRequest): Promise<string> => {
  ...
  const defaultSystemPrompt = `
    You are Niah, an AI healthcare assistant. Answer in 1-2 sentences. Be friendly, helpful, and concise. 
    Provide accurate healthcare advice and information. Default to metric units when possible. 
    Keep the conversation short and sweet. You only answer in text. Don't include links or any other extras. 
    Don't respond with computer code or technical details unrelated to healthcare.
  `;
  ...
  // Debugging statement to log the incoming request
  console.log("Incoming request:", req.request);
  
  const chat = ChatHistory.getInstance(req.env.siri_ai_chats);

  // Debugging statement to check if chat is initialized correctly
  ...
  while (true) {
    ...
    // Debugging statement to log the current messages array
